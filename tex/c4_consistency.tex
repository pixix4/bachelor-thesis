...
\begin{figure}
    ...
    \begin{tikzpicture}[>={Latex[length=3mm]}]
        ...
        \draw (-7,1) -- (-7,4);
        \draw (-7.5,2.5) node {\rotatebox{90}{\textbf{Kunde}}};
        \draw (-6,2.5) circle [radius=0.5];
        \draw (-2,2.5) circle [radius=0.5];
        \draw (-2,2.5) circle [radius=0.4];
        ...
        \draw[color=layer2] (-4.5,-5) node[anchor=north] {SequenceFlow};
        ...
        \draw[color=layer2] (2.8,-3) node[anchor=north] {CreateRelation};
        \draw[color=layer2, ->] (1,-2.3) -- (1,-4) -- (5.3,-4); 
        \draw[color=layer2] (2.8,-4) node[anchor=north] {DestroyRelation};
