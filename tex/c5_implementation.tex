...
\begin{figure}
    ...
    \begin{tikzpicture}[>={Latex[length=3mm]}]
        ...
        \draw[color=layer1, ->] (-2.5, 3) -- (-2.5,4.2) -- (2.5, 4.2) -- (2.5,2.5) -- (-1,2.5) node[anchor=south west] {0..*} node[anchor=north west] {+ children};
        \draw[color=layer1, ->] (-7.5, 3) -- (-7.5,4.2) -- (-3.5, 4.2) -- (-3.5, 3) node[anchor=south east, align=right] {0..1\\parent};

        \draw[color=layer2, ->] (-2, 0) -- (-2,-1.5) -- (1, -1.5) node[anchor=south east] {0..*} node[anchor=north east] {+ relations};
        \draw[color=layer2, ->] (1.5, -1) -- (1.5,0.5) -- (-1, 0.5) node[anchor=south west] {1} node[anchor=north west] {+ target};
        \draw[color=layer2, ->] (2.5, -1) -- (2.5,1.5) -- (-1, 1.5) node[anchor=south west] {1} node[anchor=north west] {+ source};

        \draw[color=layer3, ->] (-7.5, 0) -- (-7.5,-1.2) -- (-3.5, -1.2) -- (-3.5, 0) node[anchor=north east, align=right] {0..*\\matchingElements};
    \end{tikzpicture}
